...
export const AboutSection = () => {
  ...
  useEffect(() => {
    const handleSetTab = (e) => {
      ...
      if (tab === 'personal' || tab === 'professional' || tab === 'approach') {
        setActiveTab(tab);
      }
    };

    window.addEventListener('about:setTab', handleSetTab);
    return () => window.removeEventListener('about:setTab', handleSetTab);
  }, []);
  ...
  // Programmatic download function (files must exist in /public)
  const handleResumeDownload = (type) => {
    const resumeFiles = {
      sde: "/Samruddhi_Gaikwad_Resume.pdf",
      ...
      <div className="container mx-auto max-w-7xl relative">
        ...
        <div className="text-center mb-16 md:mb-20 px-2 sm:px-6">
          ...
          <h1 className="text-3xl sm:text-4xl md:text-6xl lg:text-7xl font-bold mb-4 sm:mb-6">
            ...
            <span className="block text-primary animate-pulse">Data Into Impact</span>
          </h1>
          <p className="text-base sm:text-lg md:text-xl text-muted-foreground max-w-3xl mx-auto leading-relaxed">
            Building solutions that combine <span className="text-primary font-semibold">analytics</span>, <span className="text-primary font-semibold">intelligence</span>, and <span className="text-primary font-semibold">modern development</span>
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-8 md:gap-12">
              ...
              <div className="relative">
                <div className="flex flex-col md:flex-row items-center gap-6 md:gap-8">
                  ...
                  <div className="flex-1 text-center md:text-left">
                    <h2 className="text-2xl sm:text-3xl font-bold mb-1 sm:mb-2">Samruddhi Gaikwad</h2>
                    <p className="text-primary text-base sm:text-lg font-semibold mb-3 sm:mb-4">B.Tech IT Student | Data Analytics, AI/ML & Full Stack Development</p>
          ...
          <div className="space-y-6 sm:space-y-8">
            {/* Work Together */}
            <div className="bg-card/50 border border-border rounded-3xl p-6 sm:p-8 backdrop-blur-xl shadow-2xl transition-all duration-500 hover:shadow-3xl hover:border-primary/40 hover:bg-card/60">
              <h3 className="text-lg sm:text-2xl font-bold mb-4 sm:mb-6 text-center">Let&apos;s Collaborate</h3>
              <div className="flex flex-col sm:flex-row sm:space-x-4 space-y-3 sm:space-y-0">
                <a href="#contact" className="flex-1 block w-full p-3 sm:p-4 bg-primary text-primary-foreground rounded-xl text-center font-semibold transition-all duration-300 hover:bg-primary/90 hover:scale-105 hover:shadow-lg group">
                  <div className="flex items-center justify-center gap-2 sm:gap-3"><User className="h-4 sm:h-5 w-4 sm:w-5 group-hover:scale-110 transition-transform duration-300" />Discuss Opportunities</div>
                </a>
                ...
                <div className="flex-1 relative">
                  <button
                    ...
                    className="block w-full p-3 sm:p-4 border border-border rounded-xl text-center font-semibold transition-all duration-300 hover:bg-accent hover:border-primary/30 hover:scale-105 hover:shadow-lg group"
                  >
                  ...
                  <AnimatePresence>
                    {showResumeOptions && (
                      <motion.div
                        ...
                        className="absolute bottom-full mb-2 left-0 right-0 bg-background/95 backdrop-blur-sm border border-border rounded-xl shadow-2xl p-2 z-[60] min-w-[240px]"
                      >
                        <motion.button
                          onClick={() => handleResumeDownload("sde")}
              ...
              <div className="space-y-2 sm:space-y-3">
                {features.map((feature, index) => {
                  ...
                  return (
                    <motion.div 
                      key={index} 
                      className="flex items-center gap-2 sm:gap-3 p-2 sm:p-3 rounded-lg transition-all duration-300 hover:bg-background/50 hover:scale-105"
                      initial={{ opacity: 0, x: -20 }}
                      animate={{ opacity: 1, x: 0 }}
                      transition={{ delay: index * 0.1 }}
                    >
                      <div className={`p-1.5 sm:p-2 rounded-lg bg-gradient-to-r ${feature.color} text-white`}>
                        <IconComponent className="h-3 w-3 sm:h-4 sm:w-4" />
                      </div>
                      <div className="flex-1">
                        <h4 className="text-xs sm:text-sm font-medium text-foreground">{feature.title}</h4>
            ...
            <div className="bg-card/60 border border-border rounded-3xl p-4 sm:p-6 backdrop-blur-xl shadow-2xl transition-all duration-500 hover:shadow-3xl hover:border-primary/40 hover:bg-card-70">
              <div className="flex flex-col sm:flex-row items-center justify-between gap-2 sm:gap-0 mb-2 sm:mb-3">
                <div className="flex items-center gap-2 sm:gap-3">
                  ...
                  <span className="font-semibold text-xs sm:text-sm">Open to Internships</span>
                </div>
